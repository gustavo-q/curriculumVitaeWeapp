...
一个在微信里做简历的小工具：选模板、填内容、实时预览，导出 PDF / Word / PNG 长图或 JSON 备份。
...
- **工具栏只有 3 个入口**（目录 / 样式 / 更多）：撤销重做收在「更多」里；基本信息与照片**不占工具栏**——点纸面上的姓名 / 信息栏进基本信息，点照片或「＋ 添加照片」占位进照片。同一件事只留一个入口，320px 屏上每格从 45.7px 宽到 106.7px
- **三种条目排版**：界面上叫「一行文字 / 左右两栏 / 左中右三栏」并各配一张示意图，字段名是「标题 / 说明 / 时间」，且**只显示当前排版真的会画出来的那几个输入框**（数据层仍是 ONE / TWO / THREE，换排版不丢内容）
- **列表用手势而不是按钮**：目录行与条目行都是「点一下编辑 · 长按拖动排序 · 左滑露出动作」。目录行的左滑同时给出「显示 / 隐藏」与「删除」，所以区块级操作不再散落在编辑面板里；区块面板因此只剩「区块名称 + 条目列表 + 添加条目」
- **照片**：内置头像或相册选图，支持外形、边框、尺寸、证件照比例
- **自动保存**：每 4 秒落盘，离开页面或切后台再补存一次，所以没有保存按钮
- **撤销 / 重做**：连续输入合并为一步
- **A4 软分页**：自动画页缝和页码，跨页的区块整体推到下一页
- **智能压缩到一页**：逐级调整字号行距，把内容压进一页
- **导出（都在「更多」菜单 / 预览页的「导出 ▾」里）**：
  - **PDF**：逐页 A4 渲染（2 倍高清），组装成真实多页 PDF 后打开文档预览，可「用其他应用打开」转发给电脑或直接打印
  - **Word（.doc）**：按当前排版参数生成 Word 文档，交给 WPS / Word 打开后可继续编辑
  - **PNG 长图**：2 倍高清，直接保存到相册
  - **JSON 备份**：复制到剪贴板，换设备粘贴导入
...
utils/
  ...
  render.js      canvas 渲染（导出 PNG / PDF 逐页用）
  onePage.js     智能压缩到一页
  export.js      导出 / 剪贴板 / 相册等平台能力封装
  pdf.js         纯 JS 的 PDF 组装器（JPEG → 多页 PDF）
  word.js        简历 → Word(.doc) HTML 生成
tools/           开发期回归脚本（不参与打包）
```
...
- **屏幕与导出必须同源**：排版几何只在 `utils/layout.js` 里算一次，屏幕渲染、页数判定、
  PNG 导出、PDF 逐页导出共用同一套结果。改了样式常量，两边必须同步——`tools/check-geometry.cjs` 负责守住这条。
...
- **导出**：PDF / Word / PNG / JSON 四种。PDF 是「位图版式」——逐页截图嵌入，
  所见即所得且任何设备打开都不跑版，但文字不可选中（这是无后端小程序的边界；
  需要可编辑文本请用 Word 导出）。Word 是「Word HTML」格式，WPS / Word 打开后
  可编辑、可另存为 .docx，复杂装饰（如标题色块圆角）会取近似样式。
